...
def pr2_mover(object_list):
    ...
    for i in range(len(dropbox_param)):			# Here extract each dropbox's position
    ...
    for object in object_list: 			# Here we loop through detected objects, extract their centroids and respective labels
    ...
    for idx in range(len(object_list_param)): 	# We then loop through each object in the scene, and check if they exist
    											# in the list of detected objects, if so, pass the respective parameters to the 
        name = object_list_param[idx]['name']   # service request to initiate a pick and place cycle.
